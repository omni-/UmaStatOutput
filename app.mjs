...
const BASE_SPECIALTY_WEIGHT = 100,
  OFF_TRAINING_WEIGHT = 100,
  NO_TRAINING_WEIGHT = 50,
  SUPPORT_COUNT_BONUS = 0.05;
...
export function calculateMarginalTraining(card, trainingType, options = {}) {
  const gains =
    ...
    rainbow = Boolean(options.rainbow);
  let trainingBonus = Number(card.tb || 1),
    motivationBonus = Number(card.mb || 1),
    friendshipBonus = 1;
  if (rainbow) {
...
export function calculateCardEV(card, options = {}) {
  const profile =
    ...
    rainbowMarginal = calculateMarginalTraining(card, card.type, {
      gains: profile.gains[card.type],
      motivation,
      growth,
      rainbow: true,
    }),
    specialtyVector = rainbowMarginal.map(
      (value) => value * appearance.specialty,
    ),
    offVector = new Array(6).fill(0);
...
function conditionalFlags(card) {
  ...
  if ((card.crowd_bonus || 0) !== 0) flags.push("crowd-size training unique");
  if ((card.highlander_training || 0) !== 0)
    flags.push("deck-diversity training unique");
  if ((card.fan_bonus || 0) !== 0) flags.push("fan-count training unique");
  return flags;
}
...
function initBrowser() {
  const els = {
    ...
    state = { payload: null, groups: [], selected: [], activeDetailId: null },
  ...
  function restoreSettings(saved) {
    ...
    if (saved.profile && TRAINING_PROFILES[saved.profile])
      els.trainingProfile.value = saved.profile;
    ...
    els.searchResults.innerHTML = matches
      .map((group) => {
        const card =
          ...
          maxLb = Math.max(...group.lbs.keys());
        return `<button class="search-result" type="button" data-add-id="${card.id}" data-card-type="${card.type}"><div class="accent-card-thumb"><img class="card-thumb" src="${supportImageUrl(card.id)}" alt="" loading="lazy" /></div><div>${titleMarkup(card, "search-titleline")}<div class="name-row"><div class="search-name">${htmlEscape(card.char_name)}</div>${rarityMarkup(card)}</div><div class="search-meta">${TYPE_NAMES[card.type]} · #${card.id} · LB0–${lbLabel(maxLb)}</div></div><div class="add-mark">＋</div></button>`;
      })
      .join("");
    els.searchResults.hidden = false;
  }
  function addCard(id) {
    if (state.selected.length >= 10 || state.selected.some((s) => s.id === id))
      return;
  ...
  function renderSelected() {
    ...
    if (!state.selected.length) {
      ...
      els.selectedCards.textContent =
        "Search for a card above. Cards are loaded from Euophrys' Global support dataset and default to their highest available limit break.";
      return;
    }
    ...
    els.selectedCards.innerHTML = state.selected
      .map((sel) => {
        const group = state.groups.find((g) => g.id === sel.id),
          card = currentCard(sel),
          lbOptions = [...group.lbs.keys()]
            .sort((a, b) => a - b)
            .map(
              (lb) =>
            ...
            .join("");
        return `<div class="selected-card" data-card-type="${card.type}">${portraitMarkup(card)}<div style="min-width:0">${titleMarkup(card)}<div class="name-row"><div class="selected-title" title="${htmlEscape(card.char_name)}">${htmlEscape(card.char_name)}</div>${rarityMarkup(card)}</div><div class="selected-meta"><span class="type-tag">${TYPE_NAMES[card.type]}</span><select data-lb-id="${card.id}">${lbOptions}</select></div></div><button class="remove-card" data-remove-id="${card.id}" type="button" title="Remove">×</button></div>`;
      })
      .join("");
  }
  function evaluatedCards() {
    const opts = settings(),
      rows = state.selected.map((sel) => {
  ...
  function renderDetails(row) {
    ...
    const { card, ev, flags } = row,
      warning = flags.length
        ...
        : "";
    els.resultDetails.innerHTML = `<div class="detail-card" data-card-type="${card.type}"><div class="detail-top"><div>${titleMarkup(card)}<div class="name-row"><div class="detail-title">${htmlEscape(card.char_name)}</div>${rarityMarkup(card)}</div><div class="search-meta">${TYPE_NAMES[card.type]} · ${lbLabel(card.limit_break)} · #${card.id} · Specialty Priority ${fmt(card.specialty_rate, 0)} · Specialty Rate ${pct(ev.appearance.specialty)}</div></div><div class="formula-chip">#${card.id}</div></div><div class="detail-ev-heading"><strong>Per-turn Specialty EV by stat</strong><span>Appearance-weighted average contribution across all turns.</span></div><div class="stat-vector">${STAT_NAMES.map((name, i) => `<div class="stat-cell"><span>${name}</span><strong>${fmt(ev.specialtyVector[i])}</strong></div>`).join("")}</div>${warning}</div>`;
  }
  function renderResults() {
    ...
    els.resultsBody.innerHTML = rows
      .map((row, index) => {
        const { card, ev, flags } = row;
        return `<tr data-detail-id="${card.id}" class="${state.activeDetailId === card.id ? "active" : ""}" data-card-type="${card.type}"><td class="rank">${index + 1}</td><td><div class="result-support" data-card-type="${card.type}"><div class="accent-card-thumb small"><img class="card-thumb" src="${supportImageUrl(card.id)}" alt="" /></div><div>${titleMarkup(card, "result-titleline")}<div class="name-row"><div class="result-name">${htmlEscape(card.char_name)}${flags.length ? '<span class="warn-dot" title="Context-dependent unique">◆</span>' : ""}</div>${rarityMarkup(card)}</div><div class="result-sub">${TYPE_NAMES[card.type]} · ${lbLabel(card.limit_break)} · #${card.id}</div></div></div></td><td><div class="metric-main">${fmt(card.specialty_rate, 0)}</div><div class="metric-sub">得意率</div></td><td><div class="metric-main">${pct(ev.appearance.specialty)}</div><div class="metric-sub">preferred training appearance</div></td><td><div class="metric-main">+${fmt(ev.rainbowScore)}</div><div class="metric-sub">extra weighted stats</div></td><td class="${index === 0 ? "best" : ""}"><div class="metric-main">${fmt(ev.specialtyScore)}</div><div class="metric-sub">weighted stats / turn</div></td></tr>`;
      })
      .join("");
    renderDetails(
  ...
  els.trainingProfile.addEventListener("input", () => {
    syncSpecUI(TRAINING_PROFILES[els.trainingProfile.value].globalSpecialty);
    onSettingsChanged();
  });
  [els.motivation, els.spWeight].forEach((el) =>
  ...
  fetch("./data/cards.json", { cache: "no-cache" })
    ...
    .then((payload) => {
      ...
      const date = payload.generated_at
        ...
        : "current build";
      els.sourcePill.textContent = `${state.groups.length} supports · synced ${date}`;
